...
public class HandVisual : MonoBehaviour
{
    ...
    public void HighlightCards(bool highlight)
    {
        foreach (GameObject g in CardsInHand)
        {
            g.GetComponent<OneCardManager>().IsValidTarget = highlight;
        }
    }
    ...
    GameObject CreateACardAtPosition(CardAsset c, Vector3 position, Vector3 eulerAngles)
    {
        ...
        else
        {
            ...
            if (c.targets.Contains(TargetingOptions.NoTarget))
                card = Instantiate(GlobalSettings.Instance.NoTargetSpellCardPrefab, position, Quaternion.Euler(eulerAngles)) as GameObject;
            else
            {
                card = Instantiate(GlobalSettings.Instance.TargetedSpellCardPrefab, position, Quaternion.Euler(eulerAngles)) as GameObject;
    ...
    public void PlayASpellFromHand(GameObject CardVisual)
    {
        ...
        CardActionManager.Instance.HighlightCastableCards(GlobalSettings.Instance.Players[owner]);
